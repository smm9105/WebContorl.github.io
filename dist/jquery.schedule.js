...
var end = [];
var i = 0;
var activePeriod_0, activePeriod_1, activePeriod_2, activePeriod_3; 
var xMachine=[];
...
var counter = 0;
var counter1 = 0;
var error0, error1, error2, error3;
...
var pubnub = new PubNub({
  subscribeKey: "sub-c-a8b8c192-5ad3-11e8-9796-063929a21258",
  publishKey: "pub-c-69db93be-b260-4781-b77e-75d8f4dd60a2",
  ssl: true
})


function myTimer() {
  rawdata  = $("#schedule").jqs('export');
  dataParsed = JSON.parse(rawdata);
  var d = new Date();
  var hours = d.getHours();
  var day = d.getDay() -1;

try{
  start[0] =  dataParsed[day].periods[0].start.slice(0,2);
  end[0]   =  dataParsed[day].periods[0].end.slice(0,2);
...
if(start[0] < hours.toString() && end[0] >= hours.toString())
  {activePeriod_0 = true}
  else{activePeriod_0 = false}

  try {
  start[1] =  dataParsed[day].periods[1].start.slice(0,2);
  end[1]   =  dataParsed[day].periods[1].end.slice(0,2);
...
if(start[1] < hours.toString() && end[1] >= hours.toString())
{activePeriod_1 = true}
else{activePeriod_1 = false}

try {
  start[2] =  dataParsed[day].periods[2].start.slice(0,2);
  end[2]   =  dataParsed[day].periods[2].end.slice(0,2);
...
if(start[2] < hours.toString() && end[2] >= hours.toString())
  {activePeriod_2 = true}
  else{activePeriod_2 = false}
  
try {
  start[3] =  dataParsed[day].periods[3].start.slice(0,2);
  end[3]   =  dataParsed[day].periods[3].end.slice(0,2);
...
if(start[3] < hours.toString() && end[3] >= hours.toString())
  {activePeriod_3 = true}
  else{activePeriod_3 = false}
    

    if(activePeriod_0 || activePeriod_1 || activePeriod_2 || activePeriod_3)
{
  turnOn = true;
}
else {turnOn = false}

console.log(activePeriod_0,activePeriod_1, activePeriod_2, activePeriod_3, turnOn, counter, start[1],d);
console.log(error0,error1, error2, error3);
if (turnOn == true) {counter = counter + 1}
  else{counter = 0};
if (turnOn == false) {counter1 = counter1 + 1}
  else{counter1 = 0};
  //console.log(counter);
  ...
  pubnub.publish(
    {
        message: { 
            activePeriod: turnOn,
            xMachine1:  xMachine[1],
            xMachine2:  xMachine[2],
            xMachine3:  xMachine[3],
            xMachine4:  xMachine[4],
            xMachine5:  xMachine[5],
            xMachine6:  xMachine[6],
            xMachine7:  xMachine[7],
            xMachine8:  xMachine[8],
            Date : d
        },
        channel: 'Test',
